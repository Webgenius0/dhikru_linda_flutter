...
final class GoogleSignInRx extends RxResponseInt<LoginModel> {
  final api = GoogleSignInApi.instance;

  GoogleSignInRx({required super.empty, required super.dataFetcher});

  final ValueNotifier<bool> isLoading = ValueNotifier(false);

  ValueStream get getFileData => dataFetcher.stream;

  Future<LoginModel?> googleSignInRx({
    required String accessToken,
  }) async {
    isLoading.value = true;
    try {
      final data = await api.googleLoginApi(accessToken: accessToken);
